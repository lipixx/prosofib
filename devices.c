...
sys_read_keyboard (int fd, char *buffer, int size)
{
  ...
  if ((circ_chars == size || actual->chars_pendents <= 0)
      && list_empty (&keyboard_queue))
    {
      /* Hi ha suficients caracters al buffer i no hi ha cap proces esperant dades */

      if (circ_chars == size)
	escriure = size;	/* Si esta ple ho escrivim tot */
      else
	escriure = actual->chars_pendents;	/* Si ja tenim tots els caracters que voliem els escrivim */
...
void
bloquejar_teclat (struct task_struct *task)
{
...
void
desbloquejar_teclat (struct task_struct *task)
{
...
void
llegir_al_circ_es_molt_divertit (char *origen, char *desti, int size)
{
  int i = 0;
  for (; size > 0; size--, i++)
    {
...
void
anar_al_circ (struct task_struct *bloq, int size)
{
  /* Copia les dades */
...
int
sys_read_file (int fd, char *buffer, int size)
{
...
int
sys_release_file (int dir_entry)
{
